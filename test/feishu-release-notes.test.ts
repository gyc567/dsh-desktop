...
describe('Feishu release notes pipeline', () => {
  ...
  it(
    ...
    () => {
      ...
      expect(output).toContain("You are Aura智能工作台's Release Bot.")
      expect(output).toContain('## Aura智能工作台 v0.4.0 Release Note')
      expect(output).toContain('📢 大家可以直接在客户端中更新。')
      expect(output).toContain('📢 You can update directly from the Aura智能工作台 app.')
  ...
  it('generates deterministic fallback release notes that pass validation', () => {
    ...
    try {
      ...
      expect(content).toContain('## Aura智能工作台 v0.4.0 Release Note')
      expect(content).toContain('📢 大家可以直接在客户端中更新。')
      expect(content).toContain('📢 You can update directly from the Aura智能工作台 app.')
  ...
  it('rejects invalid markdown with links or incorrect headings', () => {
    ...
    try {
      ...
      const invalidContent = `## Aura智能工作台 v0.4.0 Release Note
...
## Aura智能工作台 v0.4.0 Release Note

📢 You can update directly from the Aura智能工作台 app.
...
`
  ...
  it('generates deterministic fallback prerelease notes that pass validation', () => {
    ...
    try {
      ...
      const content = readFileSync(tempFile, 'utf8')
      expect(content).toContain('## Aura智能工作台 v0.7.2（预发布）Release Note')
  ...
  it(
    ...
    () => {
      ...
      expect(output).toContain('## Aura智能工作台 v0.7.2（预发布）Release Note')
