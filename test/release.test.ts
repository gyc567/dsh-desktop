...
describe('GitHub release contract', () => {
  ...
  it('uses stable platform-specific artifact names', async () => {
    ...
    expect(packageJson.build.artifactName).toBe('aura-${os}-${arch}.${ext}')
  ...
  it('shows a packaged startup surface and pins the Electron directory picker surface', async () => {
    ...
    expect(splash).toContain('Starting Aura智能工作台')
  ...
  it('publishes update metadata for installed desktop builds', async () => {
    ...
    for (const asset of [
      ...
      'latest-mac.yml',
      'latest.yml',
      'aura-mac-arm64.zip.blockmap',
      'aura-mac-x64.zip.blockmap',
      'aura-windows-x64-setup.exe.blockmap'
    ]) {
  ...
  it('packages an isolated development channel from the current workspace', async () => {
    ...
    expect(developmentConfig).toContain("productName: 'Aura Dev'")
  ...
  it('builds and publishes every supported platform', async () => {
    ...
    expect(workflow).toContain("'dist-dev\\win-unpacked\\Aura Dev.exe'")
    ...
    expect(workflow).toContain('dist-dev/aura-dev-windows-x64-setup.exe')
