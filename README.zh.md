...
![Aura智能工作台 的 Preset、模型提供方、手机控制与可编辑 PPT 生成功能](docs/images/dsh-desktop-hero-v021.png)

<p align="center"><strong>使用 DeepSeek 官方模型或主流第三方模型，管理可移植的 Agent Preset，在手机上继续 Harness 会话，并将材料生成可继续编辑的 PPTX。</strong></p>

Aura智能工作台 把本地 DeepSeek Harness 封装为可安装的桌面应用。它会自动启动 Harness，把 Profile、插件、工作区、模型配置和会话保存在应用安装目录之外，并在本地 Runtime 就绪后直接进入完整 Harness 界面。

> [!IMPORTANT]
> Aura智能工作台 当前处于早期预览阶段，基于仍在快速迭代的 `@deepseek-ai/dsh@0.1.2-rc.1`。macOS 正式包已完成代码签名并通过 Apple 公证；Windows x64 安装包也已完成代码签名。随着下载量、安装量和发行者信誉逐步积累，Windows 安全提示会逐渐减少，但不会立即消失。
...
安装版会在启动后及每六小时检查更新。发现新版本时，Aura智能工作台 会先询问用户；同意后才开始下载，只有选择“重新启动并安装”后才会进入安装。你也可以从应用菜单手动检查，或跳过当前版本而不影响后续版本提示。

## 加入社区

<p align="center">
  使用微信扫描下方二维码，加入 Aura智能工作台 微信交流群。<br />
  <img src="docs/images/wechat-group-20260815.png" width="220" alt="Aura智能工作台 微信群二维码" /><br />
  也可以加入 <a href="https://discord.gg/he2gAKCpj">Aura智能工作台 Discord 社区</a>。
</p>

## Aura智能工作台 带来了什么

DeepSeek Harness 已经提供 Agent Runtime 与 Web UI。Aura智能工作台 在此基础上补齐真正的桌面宿主能力：
...
如果第三方插件导致启动或页面渲染异常，Aura智能工作台 会结合 Runtime 与前端证据定位相关插件，并打开引导式恢复界面。
...
Aura智能工作台 采用 [MIT License](LICENSE) 开源。

DeepSeek Harness 及其依赖仍遵循各自的上游许可证与商标规则。Aura智能工作台 是独立的社区桌面应用。
