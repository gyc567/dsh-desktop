...
PROMPT_TEMPLATE = """\
You are Aura智能工作台's Release Bot. Rewrite the source release note as polished,
...
def build_prompt(release_tag: str, prerelease: bool = False) -> str:
    ...
    heading = (
        f"## Aura智能工作台 v{version}（预发布）Release Note"
        if prerelease
        else f"## Aura智能工作台 v{version} Release Note"
    )
    zh_notice = (
        "⚠️ 本次为预发布版本，供测试与体验使用。"
        if prerelease
    ...
    en_notice = (
        ...
        if prerelease
        else "📢 You can update directly from the Aura智能工作台 app."
    )
...
def send_feishu_notification(
    ...
    card_title = (
        f"🧪 Aura智能工作台 {release_tag}（预发布）已发布"
        if prerelease
        else f"✅ Aura智能工作台 {release_tag} 发布成功"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Feishu release notes tool for Aura智能工作台")
